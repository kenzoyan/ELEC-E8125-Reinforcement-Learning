...
def parse_args(args=sys.argv[1:]):
    ...
    parser.add_argument("--min-update-samples", type=int, default=5000,
...
def train(agent, env, train_episodes, render=False, silent=False, 
    ...
    num_updates = 0
    print("train_episodes: "+ str(train_episodes))
    # Run actual training
    for episode_number in range(train_episodes):
        ...
        while not done:
            ...
            reward = new_reward_point(previous_observation, action, observation, env)
            # reward = new_reward_velocity(previous_observation, action, observation, env)
...
def test(agent, env, episodes, render=False):
    ...
    print("test_episodes: "+ str(episodes))
    # allEpisodeRewards=[]
    for ep in range(episodes):
        ...
        while not done:
            ...
            # reward=new_reward_point(observation)
            if render:
                env.render()
        ...
        # allEpisodeRewards.append[test_reward]
        print("Test ep reward in episode"+str(ep)+":", test_reward)
    print("Average test reward:", total_test_reward/episodes, "episode length:", total_test_len/episodes) 


# TODO: Definition of the modified reward function   new_reward(previous_observation, action, observation, env)
