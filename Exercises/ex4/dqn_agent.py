...
class Agent(object):
    def __init__(self, env_name, state_space, n_actions, replay_buffer_size=50000,
        ...
        if "CartPole" in self.env_name:
            ...
            self.policy_net = LunarLanderDQN(state_space, n_actions, hidden_size)
            self.target_net = LunarLanderDQN(state_space, n_actions, hidden_size)
            self.target_net.load_state_dict(self.policy_net.state_dict())
            self.target_net.eval()
    ...
    def get_action(self, state, epsilon=0.05):
        ...
        # Hint:
        # If greedy
        #   convert state to torch float tensor
        #   get q values from policy net (consider using with torch.no_grad()
        #   return action
        # If not greedy
        ...
        if random.random()< epsilon:
            ...
            with torch.no_grad():
                ...
                qmax_action=np.argmax(qvalues).item()
                # print("qmax_action",qmax_action)
                return qmax_action
