...
class Agent(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        # TODO: Define the critic network and actor_mean.
        #  Both of them have two hidden layers with size 64 and use tanh() activation function.
        # Hint: We use the orthognoal initialization for the weight and constant for the bias.
        #       You can call layer_init(nn.Linear(dim1, dim2), std) to correctly init the nn.
        ...
        self.actor_mean = nn.Sequential(
            ...
            layer_init(nn.Linear(64, 32)),
            nn.Tanh(),
            layer_init(nn.Linear(32, action_dim), std=1.0),
            nn.Tanh(),
        )
...
class PPOBuffer(object):
    def __init__(self,
        ...
        self.states, self.actions, self.logprobs, self.rewards, self.start_episodes, self.values \
    ...
    def reset(self, ):
        ...
        # self.returns ans self.advantages are needed to be calculated and
        # filled in compute_returns_and_advantages fn
    ...
    def compute_returns_and_advantages(self, last_value: torch.Tensor, done: bool):
        """ Compute the returns and advantage according to GAE formula,
        see (11) (12) in https://arxiv.org/pdf/1707.06347.pdf """
        ...
        # returns = []
        gae = 0
        rewards, values, masks = self.rewards, self.values, self.start_episodes
        last_value = last_value.cpu().numpy()
        for step in reversed(range(len(rewards))):
            delta = rewards[step] + self.gamma * last_value * (1 - done) - values[step]
...
class PPO(object):
    ...
    def collect_data(self, num_timesteps_per_env):
        """ Collect data by interacting with environments, and for each environment, we collect num_timesteps_per_env. 
            Thus, the total env timesteps collected are num_envs * num_timesteps_per_env.
        ...
        for step in range(0, num_timesteps_per_env):
            ...
            # -----
            state = self.states
            # state = self.buffer._to_tensor(state)
            state = torch.FloatTensor(state).to(device)
            value = self.agent.get_value(state)
            action, logprob, entropy = self.agent.get_action(state)
            ...
            self.buffer.add(state, action, reward, start_episode, value, logprob)
            self.states = next_state
            self.start_episodes = start_episode

            # TODO: collect data from the environment and add them into buffer
            # Hint: Notice the type of a variable. Is it a numpy, a tensor on CPU or a tensor on GPU?
            # Hint: We use gym.wrappers.RecordEpisodeStatistics(env) to automatically
            # reset env at the end of one episode. See make_env().
            # Hint: Call self.buffer.add() to fill needed variables into buffer.

            # Unlike TD3, in PPO, we didn't process the done flag when episode_timesteps == env._max_episode_steps,
            # This will change the problem to obtain the maximal returns within env._max_episode_steps. You can
            # compare the difference.
            for info in infos:  # infos is the information dictionary obtained when calling env.step(): ns, r, d,
                # infos = envs.step(a)
    ...
    def train(self, ):
        ...
        for epoch in range(self.update_epochs):
            for data in self.buffer.get(self.mini_batch_size):
                ...
                # TODO: Implement the loss according to eq (7)(9)
                #  in https://arxiv.org/pdf/1707.06347.pdf
                # Hint: For the value loss, we just use the simple mse loss. 'returns' are the targets.
                # Hint: The final loss is the weighted sum of policy loss,
                # value loss and entropy loss. The weigths are 1, self.vf_coeff, self.ent_coeff.
                # Hint: The gradient can be clipped (stabilize the training) by
                ...
                # ---------------
                new_actions, new_logprobs, entropy = self.agent.get_action(states, actions)
                new_values = self.agent.get_value(states)
                prob_ratio = new_logprobs.exp() / logprobs.exp()
                weighted_probs = advantages * prob_ratio
                weighted_clipped_probs = torch.clamp(prob_ratio, 1 - self.clip_ratio, 1 + self.clip_ratio) * advantages
                # actor loss
                pg_loss = - torch.min(weighted_probs, weighted_clipped_probs).mean()
                # critic loss
                v_loss = ((returns - new_values) ** 2).mean()
                # entropy_loss
